############################################################################################
# Template for the analysis of a GADGET3 simulation. 
...
#= 
  ...
  Plots:
    - Star density field projected into the x-y plane.
    - Density profile of stars.
    - Density profile of gas.
    - Density histogram of gas.
    ...
    - Accumulated mass profile of the stars.
    - Accumulated mass profile of the gas.
    ...
    - Star mass vs. time.
    - SFR vs. time.
    - CPU usage vs. CPU step (`cs_sfr` process only).
...
density_histogram_pipeline(
    ...
    sim_cosmo = SIM_COSMO,
    flags = (
        [2.471402662718477e7 / (10^4), 2.471402662718477e8 / (10^4)],
        [L"1\,\mathrm{cm}^{-3}", L"10\,\mathrm{cm}^{-3}"],
    ),
    factor = 4,
)
