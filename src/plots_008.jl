#= 
  ...
  The resulting figures will be stored in ../../plots/008/.

  The plots have three defining characteristics: their simulation, snapshot time, 
  and age filter used. By default the function `kennicutt_schmidt_pipeline` will slice 
  the images by simulation -> age filter -> time. This is what ends up in the folder 
  `all_Kennicutt_Schmidt`. Given that we are interested in the moment of maximum SFR, 
  we slice the results as simulation -> age filter, at a fixed time (a different one for 
  each simulation). This is what ends up in the folder `Kennicutt_Schmidt_at_max_sfr`. 
...
of the files and the approximate time of maximum SFR in units of 10Myr.
"""
const SNAPSHOTS = [
    "run_00" "snap" 60
...
for (simulation, base_name, max_sfr_time) in zip(simulations, base_names, max_sfr_times)
    ...
    final_out_path = mkpath(
        joinpath(BASE_OUT_PATH, "Kennicutt_Schmidt_at_max_sfr", simulation),
    )
